...
这是 Telegram 记账机器人 Go/PostgreSQL 主线；当前源码候选版本为 v2.4.6，只有显式发布 workflow 成功后才会产生正式镜像、tag 和 Release。生产部署使用 PostgreSQL、GHCR 镜像和共享 `ledger-chain-watcher` 链上监听服务。

v2.4.6 发布说明见 [docs/releases/v2.4.6.md](docs/releases/v2.4.6.md)，生产升级、回滚限制和验收见 [docs/production-rollout-v2.4.6.md](docs/production-rollout-v2.4.6.md)。

## v2.4.6 发布范围

- watcher 使用每 Key 可持续令牌预算、独立 P1 保留通道和受限 P2-PN 持久化，额度预测只用于调度，真实 429 仍是权威反馈。
- 锚点扩页与时间窗补偿改为可持久续跑的 gap task，支持重叠归并、`retry_after`、公平老化和动态 catch-up worker。
- `/status` 增加 gap scheduler 指标；链上通知以 priority 0 原子入 outbox 并立即唤醒 critical 发送通道。
- 新增前向迁移 `2.4.6-chain-gap-scheduler`、ready-claim 索引和分钟级 gap 指标表。
- bot 与 watcher 均有代码变更，生产必须先更新 watcher、确认 ready，再同步更新 bot。
...
正式发布后，使用 Go v2.4.6 镜像、PostgreSQL 和共享 watcher：
...
宝塔 Docker Compose 可以直接使用仓库里的 [docker-compose.ghcr.yml](docker-compose.ghcr.yml)。这个文件默认拉取 `ghcr.io/dayou0168/telegram-ledger-bot-go:2.4.6` 和 `ghcr.io/dayou0168/telegram-ledger-chain-watcher:2.4.6`，在同一个 Compose 项目里用独立 PostgreSQL 容器和独立 watcher 容器组成全家桶。在 Release 可用前不要在生产执行该模板。
...
如果希望 `ledger-chain-watcher` 直接跑在宿主机 systemd 里，v2.4.6 正式 Release 会同时发布 `ledger-chain-watcher-v2.4.6-linux-amd64.tar.gz` 宿主机包，里面包含二进制、[deploy/ledger-chain-watcher.env.example](deploy/ledger-chain-watcher.env.example) 和 [deploy/ledger-chain-watcher.service](deploy/ledger-chain-watcher.service)。本轮 watcher 与 bot 必须同步升级；机器人 Compose 保留自己的 `DATABASE_URL`，并把 `CHAIN_WATCHER_URL` 配成 `http://host.docker.internal:8090` 或 Docker 网桥 IP。
...
Go v2.4.6 通过共享 `ledger-chain-watcher` 获取链上数据。多个机器人实例只配置内网 URL 和内部密钥，不再各自配置官网 API Key：
...
普通 push 只运行 CI。脚本只有在 PostgreSQL CI、race 和静态构建全部成功后才调用 `workflow_dispatch(version=2.4.6)`；任一步失败都会停止，不会自动部署。

服务器部署见 [docs/deployment.md](docs/deployment.md)。v2.4.6 必须在显式发布 workflow 成功、实际 digest 和校验和可用后，再按 [生产升级、回滚与验收](docs/production-rollout-v2.4.6.md) 先更新 watcher、确认 ready，再更新 bot。外层旧工作区部署文件不得回灌，唯一候选基线是本集成仓库的已确认发布提交。
