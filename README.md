...
这是 Telegram 记账机器人 Go/PostgreSQL 主线；当前源码候选版本为 v2.4.12，只有显式发布 workflow 成功后才会产生正式镜像、tag 和 Release。生产部署使用 PostgreSQL、GHCR 镜像和共享 `ledger-chain-watcher` 链上监听服务。

v2.4.12 发布说明见 [docs/releases/v2.4.12.md](docs/releases/v2.4.12.md)，生产升级、回滚限制和验收见 [docs/production-rollout-v2.4.12.md](docs/production-rollout-v2.4.12.md)。

## v2.4.12 发布范围

- 操作员目标支持三种可靠来源：回复目标消息、Telegram 可点击昵称产生的 `text_mention`、以及机器人已记录的 `@username`。没有 username 的用户也可通过前两种方式设置；普通手打昵称和数字 UID 不作为目标。
- 群成员发现区分真实发言与 `chat_member` 身份同步：只有消息入口写入 `has_spoken=true`，`通知所有人` 不会把仅入群但未发言的成员算入目标。
- 后台新增独立“回复通知”标签。广播发送者强制接收；宿主、默认操作人和 active 全局操作人按权限范围逐人控制，一级仅管理自己的 active 下级且默认开启；伪造越权 POST 返回 403。
- 回复偏好采用批量查询，读取失败时仅通知原发送者，不扩大通知范围；PC/手机布局均支持局部滚动和响应式操作。
- 新增 `2.4.16-chat-member-discovery` 与 `2.4.17-broadcast-reply-preferences` PostgreSQL 迁移；升级前必须备份机器人数据库。
...
正式发布后，使用 Go v2.4.12 镜像、PostgreSQL 和共享 watcher：
...
宝塔 Docker Compose 可以直接使用仓库里的 [docker-compose.ghcr.yml](docker-compose.ghcr.yml)。这个文件默认拉取 `ghcr.io/dayou0168/telegram-ledger-bot-go:2.4.12` 和 `ghcr.io/dayou0168/telegram-ledger-chain-watcher:2.4.12`，在同一个 Compose 项目里用独立 PostgreSQL 容器和独立 watcher 容器组成全家桶。在 Release 可用前不要在生产执行该模板。
...
如果希望 `ledger-chain-watcher` 直接跑在宿主机 systemd 里，v2.4.12 Release 仍会提供配套宿主机包；但本次功能不改变 watcher 协议，现有 v2.4.11 watcher 可以保持运行，只升级机器人镜像即可。
...
Go v2.4.12 通过共享 `ledger-chain-watcher` 获取链上数据。多个机器人实例只配置内网 URL 和内部密钥，不再各自配置官网 API Key：
...
```powershell
pwsh -File scripts/publish-release.ps1 -Version 2.4.12
```

普通 push 只运行 CI。脚本只有在 PostgreSQL CI、race 和静态构建全部成功后才调用 `workflow_dispatch(version=2.4.12)`；任一步失败都会停止，不会自动部署。

服务器部署见 [docs/deployment.md](docs/deployment.md)。v2.4.12 必须在显式发布 workflow 成功且机器人镜像 digest 可用后，再按 [生产升级、回滚与验收](docs/production-rollout-v2.4.12.md) 逐个升级机器人；本轮不要重启 watcher。外层旧工作区部署文件不得回灌，唯一候选基线是本集成仓库的已确认发布提交。
