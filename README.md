...
这是 Telegram 记账机器人 Go/PostgreSQL 主线；当前源码候选版本为 v2.4.13，只有显式发布 workflow 成功后才会产生正式镜像、tag 和 Release。生产部署使用 PostgreSQL、GHCR 镜像和共享 `ledger-chain-watcher` 链上监听服务。

v2.4.13 发布说明见 [docs/releases/v2.4.13.md](docs/releases/v2.4.13.md)，生产升级、回滚限制和验收见 [docs/production-rollout-v2.4.13.md](docs/production-rollout-v2.4.13.md)。

## v2.4.13 发布范围

- 后台登录只信任 Telegram ticket 中的 UID，并在每次请求实时重查宿主/全局操作人角色；单群操作人不能进入后台，越权 primary/secondary POST 返回 403。
- `ADMIN_SESSION_SECRET` 成为独立且必需的会话签名密钥；缺失时后台 fail-closed。`ADMIN_WEB_TOKEN` 仅是可选第二因素，不能单独登录或签发会话。
- 宿主可把 secondary 的广播消息或回复通知独立授权给非直属 primary 观察；宿主、直属 primary、观察 primary 三类收件人各自独立开关。
- secondary 广播及其回复的文字、图片、图片说明通过可靠 outbox 持久化，支持去重、重试、原消息映射和安全清理；广播目标不再依赖 30 分钟私聊临时态，重启后仍可恢复。
- `关闭日切` 与 `设置日切-1` 都表示关闭日切：active 账期跨天/重启继续，stopped 群不会被自动启动。
- 新增 `2.4.18-operator-message-observers-admin-identity` 与 `2.4.19-broadcast-delivery-state` PostgreSQL 迁移；升级前必须备份机器人数据库。
...
正式发布后，使用 Go v2.4.13 镜像、PostgreSQL 和共享 watcher：

```powershell
docker pull ghcr.io/dayou0168/telegram-ledger-bot-go:2.4.13
docker pull ghcr.io/dayou0168/telegram-ledger-chain-watcher:2.4.13
```

宝塔 Docker Compose 可以直接使用仓库里的 [docker-compose.ghcr.yml](docker-compose.ghcr.yml)。这个文件默认拉取 `ghcr.io/dayou0168/telegram-ledger-bot-go:2.4.13` 和 `ghcr.io/dayou0168/telegram-ledger-chain-watcher:2.4.13`，在同一个 Compose 项目里用独立 PostgreSQL 容器和独立 watcher 容器组成全家桶。在 Release 可用前不要在生产执行该模板。
...
如果希望 `ledger-chain-watcher` 直接跑在宿主机 systemd 里，v2.4.13 Release 仍会提供配套宿主机包；但本次功能不改变 watcher 协议，现有 v2.4.12 watcher 可以保持运行，只升级机器人镜像即可。
...
Go v2.4.13 通过共享 `ledger-chain-watcher` 获取链上数据。多个机器人实例只配置内网 URL 和内部密钥，不再各自配置官网 API Key：
...
普通 push 只运行 CI。脚本只有在 PostgreSQL CI、race 和静态构建全部成功后才调用 `workflow_dispatch(version=2.4.13)`；任一步失败都会停止，不会自动部署。

服务器部署见 [docs/deployment.md](docs/deployment.md)。v2.4.13 必须在显式发布 workflow 成功且机器人镜像 digest 可用后，再按 [生产升级、回滚与验收](docs/production-rollout-v2.4.13.md) 逐个升级机器人；本轮不要重启 watcher。外层旧工作区部署文件不得回灌，唯一候选基线是本集成仓库的已确认发布提交。
