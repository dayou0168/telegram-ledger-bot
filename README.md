...
这是 Telegram 记账机器人 Go/PostgreSQL 主线；当前源码候选版本为 v2.4.7，只有显式发布 workflow 成功后才会产生正式镜像、tag 和 Release。生产部署使用 PostgreSQL、GHCR 镜像和共享 `ledger-chain-watcher` 链上监听服务。

v2.4.7 发布说明见 [docs/releases/v2.4.7.md](docs/releases/v2.4.7.md)，生产升级、回滚限制和验收见 [docs/production-rollout-v2.4.7.md](docs/production-rollout-v2.4.7.md)。

## v2.4.7 发布范围

- 已推进的 gap 在重叠归并或 normalize 时不再重置 `next_page`；仅追加一个尚未开始的 successor，保证恢复任务单调收敛。
- fair lane 按最后处理时间轮转并新增 fair-claim 索引，避免最老大任务长期饿死其他时间窗。
- catch-up 改为 250ms token-aware 调度；未实际调用 API 的 defer 不再计为失败。
- `/status` 与 `/readyz` 分离 `source_ready`、`continuity_ready`；仅历史 gap 积压不会触发 bot 公共 fallback。
- 新增前向迁移 `2.4.7-chain-gap-convergence`；bot 与 watcher 必须同步升级。
...
正式发布后，使用 Go v2.4.7 镜像、PostgreSQL 和共享 watcher：
...
宝塔 Docker Compose 可以直接使用仓库里的 [docker-compose.ghcr.yml](docker-compose.ghcr.yml)。这个文件默认拉取 `ghcr.io/dayou0168/telegram-ledger-bot-go:2.4.7` 和 `ghcr.io/dayou0168/telegram-ledger-chain-watcher:2.4.7`，在同一个 Compose 项目里用独立 PostgreSQL 容器和独立 watcher 容器组成全家桶。在 Release 可用前不要在生产执行该模板。
...
如果希望 `ledger-chain-watcher` 直接跑在宿主机 systemd 里，v2.4.7 正式 Release 会同时发布 `ledger-chain-watcher-v2.4.7-linux-amd64.tar.gz` 宿主机包，里面包含二进制、[deploy/ledger-chain-watcher.env.example](deploy/ledger-chain-watcher.env.example) 和 [deploy/ledger-chain-watcher.service](deploy/ledger-chain-watcher.service)。本轮 watcher 与 bot 必须同步升级；机器人 Compose 保留自己的 `DATABASE_URL`，并把 `CHAIN_WATCHER_URL` 配成 `http://host.docker.internal:8090` 或 Docker 网桥 IP。
...
Go v2.4.7 通过共享 `ledger-chain-watcher` 获取链上数据。多个机器人实例只配置内网 URL 和内部密钥，不再各自配置官网 API Key：
...
普通 push 只运行 CI。脚本只有在 PostgreSQL CI、race 和静态构建全部成功后才调用 `workflow_dispatch(version=2.4.7)`；任一步失败都会停止，不会自动部署。

服务器部署见 [docs/deployment.md](docs/deployment.md)。v2.4.7 必须在显式发布 workflow 成功、实际 digest 和校验和可用后，再按 [生产升级、回滚与验收](docs/production-rollout-v2.4.7.md) 先更新 watcher、确认 `source_ready=true`，再更新 bot；`continuity_ready=false` 表示历史 gap 仍在收敛，不应单独触发 fallback。外层旧工作区部署文件不得回灌，唯一候选基线是本集成仓库的已确认发布提交。
