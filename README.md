...
这是 Telegram 记账机器人 Go/PostgreSQL 主线；当前源码候选版本为 v2.4.11，只有显式发布 workflow 成功后才会产生正式镜像、tag 和 Release。生产部署使用 PostgreSQL、GHCR 镜像和共享 `ledger-chain-watcher` 链上监听服务。

v2.4.11 发布说明见 [docs/releases/v2.4.11.md](docs/releases/v2.4.11.md)，生产升级、回滚限制和验收见 [docs/production-rollout-v2.4.11.md](docs/production-rollout-v2.4.11.md)。

## v2.4.11 发布范围

- 修复单群广播替换只配置图片时清空原图片说明文字的问题；固定替换文字为空时沿用被回复消息的原 caption，原地编辑与发送新图片兜底路径行为一致。
...
- 延续 v2.4.10 已发布的数据库迁移和 watcher 协议；本次没有新增迁移，也不要求重启生产 watcher。
...
正式发布后，使用 Go v2.4.11 镜像、PostgreSQL 和共享 watcher：
...
宝塔 Docker Compose 可以直接使用仓库里的 [docker-compose.ghcr.yml](docker-compose.ghcr.yml)。这个文件默认拉取 `ghcr.io/dayou0168/telegram-ledger-bot-go:2.4.11` 和 `ghcr.io/dayou0168/telegram-ledger-chain-watcher:2.4.11`，在同一个 Compose 项目里用独立 PostgreSQL 容器和独立 watcher 容器组成全家桶。在 Release 可用前不要在生产执行该模板。
...
如果希望 `ledger-chain-watcher` 直接跑在宿主机 systemd 里，v2.4.11 Release 仍会提供配套宿主机包；但本次修复不改变 watcher 协议，现有 v2.4.10 watcher 可以保持运行，只升级机器人镜像即可。
...
Go v2.4.11 通过共享 `ledger-chain-watcher` 获取链上数据。多个机器人实例只配置内网 URL 和内部密钥，不再各自配置官网 API Key：
...
```powershell
pwsh -File scripts/publish-release.ps1 -Version 2.4.11
```

普通 push 只运行 CI。脚本只有在 PostgreSQL CI、race 和静态构建全部成功后才调用 `workflow_dispatch(version=2.4.11)`；任一步失败都会停止，不会自动部署。

服务器部署见 [docs/deployment.md](docs/deployment.md)。v2.4.11 必须在显式发布 workflow 成功且机器人镜像 digest 可用后，再按 [生产升级、回滚与验收](docs/production-rollout-v2.4.11.md) 逐个升级机器人；本轮不要重启 watcher。外层旧工作区部署文件不得回灌，唯一候选基线是本集成仓库的已确认发布提交。
