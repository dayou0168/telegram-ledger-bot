...
这是 Telegram 记账机器人 Go/PostgreSQL 主线；当前源码候选版本为 v2.4.5，只有显式发布 workflow 成功后才会产生正式镜像、tag 和 Release。生产部署使用 PostgreSQL、GHCR 镜像和共享 `ledger-chain-watcher` 链上监听服务。

v2.4.5 发布说明见 [docs/releases/v2.4.5.md](docs/releases/v2.4.5.md)，生产升级、回滚限制和验收见 [docs/production-rollout-v2.4.5.md](docs/production-rollout-v2.4.5.md)。

## v2.4.5 发布范围

- 宿主可把广播分组管理权转给 active 一级操作人；转让使用原子事务、预期 owner 并发校验和不可变审计。
- 目标一级缺少成员群的直接广播权限时，宿主可在同一事务内精确补齐；关闭补齐时返回缺失数量且不产生部分转让。
- 后台“创建者”列显示当前管理者；转让控件只对宿主显示，一级操作人继续只能管理自己的分组。
- 广播分组顶部操作区完成 PC/手机响应式重构，冲突响应会保留已选分组、目标与补齐选项。
- 本轮必须更新 bot；watcher 代码未变，发布流程按既有规则生成同版本资产，但生产无需仅为本轮更新或重启 watcher。
...
正式发布后，使用 Go v2.4.5 镜像、PostgreSQL 和共享 watcher：
...
宝塔 Docker Compose 可以直接使用仓库里的 [docker-compose.ghcr.yml](docker-compose.ghcr.yml)。这个文件默认拉取 `ghcr.io/dayou0168/telegram-ledger-bot-go:2.4.5` 和 `ghcr.io/dayou0168/telegram-ledger-chain-watcher:2.4.5`，在同一个 Compose 项目里用独立 PostgreSQL 容器和独立 watcher 容器组成全家桶。在 Release 可用前不要在生产执行该模板。
...
如果希望 `ledger-chain-watcher` 直接跑在宿主机 systemd 里，v2.4.5 正式 Release 会同时发布 `ledger-chain-watcher-v2.4.5-linux-amd64.tar.gz` 宿主机包，里面包含二进制、[deploy/ledger-chain-watcher.env.example](deploy/ledger-chain-watcher.env.example) 和 [deploy/ledger-chain-watcher.service](deploy/ledger-chain-watcher.service)。本轮 watcher 代码未变，无需仅为 v2.4.5 重启；机器人 Compose 保留自己的 `DATABASE_URL`，并把 `CHAIN_WATCHER_URL` 配成 `http://host.docker.internal:8090` 或 Docker 网桥 IP。
...
Go v2.4.5 通过共享 `ledger-chain-watcher` 获取链上数据。多个机器人实例只配置内网 URL 和内部密钥，不再各自配置官网 API Key：
...
```powershell
pwsh -File scripts/publish-release.ps1 -Version 2.4.5
```

普通 push 只运行 CI。脚本只有在 PostgreSQL CI、race 和静态构建全部成功后才调用 `workflow_dispatch(version=2.4.5)`；任一步失败都会停止，不会自动部署。

服务器部署见 [docs/deployment.md](docs/deployment.md)。v2.4.5 必须在显式发布 workflow 成功、实际 digest 和校验和可用后，再按 [生产升级、回滚与验收](docs/production-rollout-v2.4.5.md) 更新 bot。watcher 代码未变，本轮不要求更新或重启。外层旧工作区部署文件不得回灌，唯一候选基线是本集成仓库的已确认发布提交。
