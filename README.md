...
- `Z0` 查询 OKX OTC 商家所有实时汇率 TOP 10，方向为 USDT 卖 CNY；`Z1 -0.1` 按第 1 档下浮 0.1 设置入款汇率，`Z1 +0.1` 则上浮 0.1。
...
群里发送 `Z0` 会显示 `OKX OTC商家所有实时汇率 TOP 10`，不显示限额。发送 `Z1 -0.1` 会取第 1 档价格减 0.1 后写入当前群入款汇率；`Z1 +0.1` 会取第 1 档价格加 0.1。
