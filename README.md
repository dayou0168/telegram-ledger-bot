...
- 广播权限和记账权限分开：宿主/默认操作人拥有全部广播权限；可私聊 `添加广播操作人 123456 备注` 创建一级操作人，一级操作人可以再创建一层下级；分配权限支持 `授权分组 123456 财务` 和 `授权单群 123456 -100111`。
- 一级操作人只能把自己已有的分组或单群继续分配给下级；下级不能继续创建操作人。操作人创建的分组会自动授权给自己。
- 私聊 `单群广播 -100111 广播内容` 可只发送到一个已授权群。
- 群发广播和分组广播都支持 `通知所有人`，会在目标群内 @ 所有有过发言记录的用户，例如 `群发广播 通知所有人 广播内容`。
- 群成员回复机器人投递出去的广播消息时，机器人会按发送任务记录通知原广播操作人和宿主。
- 广播替换支持 `设置替换文字`、回复图片发送 `设置替换图片`、`开启广播替换`、`关闭广播替换`、`清空广播替换`。开启后，广播发送时使用设置好的图片/文字替换原始内容；关闭后恢复原始广播。
- 地址白名单：群内操作人可用 `添加白名单地址 Txxxx 备注`、`删除白名单地址 Txxxx`、`显示白名单` 管理；群成员发送白名单地址时会回复白名单信息和验证次数。
...
宝塔里给域名申请 SSL 后，把站点反向代理到 `http://127.0.0.1:8080`。`BILL_WEB_TOKEN` 可留空，留空时网页是公开链接；建议正式使用时设置一串随机字符。`ADMIN_WEB_TOKEN` 用于打开 `/admin` 后台管理页，建议设置为另一串随机字符。

后台入口：

```text
https://bot.your-domain.example/admin?admin_token=change-this-admin-token
```

宿主或默认操作人也可以在私聊菜单点击 `⚙后台管理` 打开后台；普通广播操作人不会获得后台入口。

后台可管理地址白名单、广播操作人、广播分组、分组/单群权限、广播替换开关和已保存群组。
