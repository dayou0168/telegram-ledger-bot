...
这是 Telegram 记账机器人 Go/PostgreSQL 主线；当前源码候选版本为 v2.4.8，只有显式发布 workflow 成功后才会产生正式镜像、tag 和 Release。生产部署使用 PostgreSQL、GHCR 镜像和共享 `ledger-chain-watcher` 链上监听服务。

v2.4.8 发布说明见 [docs/releases/v2.4.8.md](docs/releases/v2.4.8.md)，生产升级、回滚限制和验收见 [docs/production-rollout-v2.4.8.md](docs/production-rollout-v2.4.8.md)。

## v2.4.8 发布范围

- 修复并发扫描轮次提前完成后，父轮次 `context` 取消被误记为 Tronscan Key transport failure 的问题。
- 由父轮次主动取消的请求不再错误冷却健康 Key；真实 HTTP 超时、网络错误和 5xx 仍按原策略计入 Key 健康与冷却。
- 本补丁不新增数据库迁移，不改变 bot 业务、readiness 或 fallback 契约。
- 生产只需更新 watcher；现有 bot v2.4.7 可以保持运行。发布流水线仍会产出同版本 bot 镜像用于完整、可复现的发布资产。
...
正式发布后，使用 Go v2.4.8 镜像、PostgreSQL 和共享 watcher：

```powershell
docker pull ghcr.io/dayou0168/telegram-ledger-bot-go:2.4.8
docker pull ghcr.io/dayou0168/telegram-ledger-chain-watcher:2.4.8
```

宝塔 Docker Compose 可以直接使用仓库里的 [docker-compose.ghcr.yml](docker-compose.ghcr.yml)。这个文件默认拉取 `ghcr.io/dayou0168/telegram-ledger-bot-go:2.4.8` 和 `ghcr.io/dayou0168/telegram-ledger-chain-watcher:2.4.8`，在同一个 Compose 项目里用独立 PostgreSQL 容器和独立 watcher 容器组成全家桶。在 Release 可用前不要在生产执行该模板。
...
如果希望 `ledger-chain-watcher` 直接跑在宿主机 systemd 里，v2.4.8 正式 Release 会同时发布 `ledger-chain-watcher-v2.4.8-linux-amd64.tar.gz` 宿主机包，里面包含二进制、[deploy/ledger-chain-watcher.env.example](deploy/ledger-chain-watcher.env.example) 和 [deploy/ledger-chain-watcher.service](deploy/ledger-chain-watcher.service)。本轮生产只需更新 watcher；机器人 Compose 可继续使用 v2.4.7 bot，并保留自己的 `DATABASE_URL`，把 `CHAIN_WATCHER_URL` 配成 `http://host.docker.internal:8090` 或 Docker 网桥 IP。
...
Go v2.4.8 通过共享 `ledger-chain-watcher` 获取链上数据。多个机器人实例只配置内网 URL 和内部密钥，不再各自配置官网 API Key：
...
```powershell
pwsh -File scripts/publish-release.ps1 -Version 2.4.8
```

普通 push 只运行 CI。脚本只有在 PostgreSQL CI、race 和静态构建全部成功后才调用 `workflow_dispatch(version=2.4.8)`；任一步失败都会停止，不会自动部署。

服务器部署见 [docs/deployment.md](docs/deployment.md)。v2.4.8 必须在显式发布 workflow 成功、实际 digest 和校验和可用后，再按 [生产升级、回滚与验收](docs/production-rollout-v2.4.8.md) 只更新 watcher 并确认 `source_ready=true`；bot v2.4.7 可保持运行，`continuity_ready=false` 仍只表示历史 gap 正在收敛。外层旧工作区部署文件不得回灌，唯一候选基线是本集成仓库的已确认发布提交。
