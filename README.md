...
这是 Telegram 记账机器人 Go/PostgreSQL 主线；当前源码候选版本为 v2.4.3，只有显式发布 workflow 成功后才会产生正式镜像、tag 和 Release。生产部署使用 PostgreSQL、GHCR 镜像和共享 `ledger-chain-watcher` 链上监听服务。

v2.4.3 发布说明见 [docs/releases/v2.4.3.md](docs/releases/v2.4.3.md)，生产升级、回滚限制和验收见 [docs/production-rollout-v2.4.3.md](docs/production-rollout-v2.4.3.md)。
...
正式发布后，使用 Go v2.4.3 镜像、PostgreSQL 和共享 watcher：

```powershell
docker pull ghcr.io/dayou0168/telegram-ledger-bot-go:2.4.3
docker pull ghcr.io/dayou0168/telegram-ledger-chain-watcher:2.4.3
```

宝塔 Docker Compose 可以直接使用仓库里的 [docker-compose.ghcr.yml](docker-compose.ghcr.yml)。这个文件默认拉取 `ghcr.io/dayou0168/telegram-ledger-bot-go:2.4.3` 和 `ghcr.io/dayou0168/telegram-ledger-chain-watcher:2.4.3`，在同一个 Compose 项目里用独立 PostgreSQL 容器和独立 watcher 容器组成全家桶。在 Release 可用前不要在生产执行该模板。
...
如果希望 `ledger-chain-watcher` 直接跑在宿主机 systemd 里，v2.4.3 正式 Release 会同时发布 `ledger-chain-watcher-v2.4.3-linux-amd64.tar.gz` 宿主机包，里面包含二进制、[deploy/ledger-chain-watcher.env.example](deploy/ledger-chain-watcher.env.example) 和 [deploy/ledger-chain-watcher.service](deploy/ledger-chain-watcher.service)。机器人 Compose 保留自己的 `DATABASE_URL`，并把 `CHAIN_WATCHER_URL` 配成 `http://host.docker.internal:8090` 或 Docker 网桥 IP。
...
Go v2.4.3 通过共享 `ledger-chain-watcher` 获取链上数据。多个机器人实例只配置内网 URL 和内部密钥，不再各自配置官网 API Key：
...
```powershell
pwsh -File scripts/publish-release.ps1 -Version 2.4.3
```

普通 push 只运行 CI。脚本只有在 PostgreSQL CI、race 和静态构建全部成功后才调用 `workflow_dispatch(version=2.4.3)`；任一步失败都会停止，不会自动部署。

服务器部署见 [docs/deployment.md](docs/deployment.md)。v2.4.3 必须在显式发布 workflow 成功、实际 digest 和校验和可用后，再按 [生产升级、回滚与验收](docs/production-rollout-v2.4.3.md) 以 watcher 先、bot 后的顺序升级。外层旧工作区部署文件不得回灌，唯一候选基线是 `codex/v2.4.3-integration` 的已确认提交。
