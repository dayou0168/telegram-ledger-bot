...
- 私聊点击 `📡群发广播` 或 `📣分组广播` 后，可用按钮选择全部群、分组或单群；选择目标后直接发送文字、图片、文件或图文，机器人会先生成确认按钮。
- 广播分组、分组成员、单群授权、广播操作人、地址白名单和广播替换都放到后台管理页操作，后台可按群名搜索或多选群组，避免手动查群 ID。
...
- 广播替换在后台管理中维护，可开启/关闭并设置替换图片、替换文字；开启后广播发送时使用设置好的图片/文字替换原始内容，关闭后恢复原始广播。
- 地址白名单在后台管理中维护；群成员发送白名单地址时会回复白名单信息和验证次数。
- 地址监听：私聊点击 `🔔地址监听` 打开按钮面板，可按钮式设置地址、删除地址、设置备注、设置最小提醒金额；USDT 监听只支持 TRC20，收入/支出/TRX 通知可单独开关。
...
宿主或默认操作人也可以在私聊菜单点击 `⚙后台管理` 打开后台，打开后输入 `ADMIN_WEB_TOKEN` 设置的后台密码。普通广播操作人不会获得全局后台入口。
