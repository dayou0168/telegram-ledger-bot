...
这是 Telegram 记账机器人 Go/PostgreSQL 主线；当前源码版本为 v2.4.14。正式镜像、tag 和 Release 只由显式发布 workflow 产生，生产部署使用 PostgreSQL、GHCR 镜像和共享 `ledger-chain-watcher` 链上监听服务。

v2.4.14 发布说明见 [docs/releases/v2.4.14.md](docs/releases/v2.4.14.md)，生产升级、回滚限制和验收见 [docs/production-rollout-v2.4.14.md](docs/production-rollout-v2.4.14.md)。

## v2.4.14 发布范围

- 上级抄送同时包含广播发送人和实际群/分组/全群目标；群成员回复回流保留原广播上下文、真实文字/图片/caption，并正确挂接上游消息。
- 单群广播替换矩阵补齐：纯文字原消息可以替换为固定图片，图片、文字、caption 组合与兜底发送路径保持一致。
- 取消广播“发送中”占位消息，只发送最终成功或失败回执，避免私聊状态噪声。
- 广播发送与回复改为个人双偏好，并受 observer ceiling 限制；发送者、直属上级、宿主和授权观察者按最终资格独立解析。
- 快速回复资格等于最终回复收件人，入队前与实际发送前均重新检查当前权限和偏好；`chat`、`group`、`all` 按钮严格遵循各自目标规则。
- 新增 `2.4.20-broadcast-message-preferences` PostgreSQL 迁移；升级前必须备份机器人数据库。
- watcher 协议和链上逻辑没有变化。发布流水线仍生成同版本 watcher 资产，但现有生产 watcher 无需重启，生产只需逐个升级机器人。
...
- `添加操作员 @user`、`删除操作员 @user`，也支持回复对方消息或选择 Telegram 可点击昵称后发送命令；普通手打昵称和数字 UID 不作为操作员目标。
...
- 群成员回复机器人投递出去的广播消息时，机器人会通知原广播发送者；后台“回复通知”可在权限范围内控制额外接收人，发送者本人的通知不能关闭。
...
- 历史版本中的 `cutoff_hour=0` 与用户明确设置的 00:00 日切无法区分，禁止自动批量改成 `-1`。确认旧群原本确实需要关闭日切后，只能让该群重新发送 `设置日切-1` / `关闭日切`，或由运维按确认白名单定向修正。
...
正式发布后，使用 Go v2.4.14 镜像、PostgreSQL 和共享 watcher：

```powershell
docker pull ghcr.io/dayou0168/telegram-ledger-bot-go:2.4.14
docker pull ghcr.io/dayou0168/telegram-ledger-chain-watcher:2.4.14
```

宝塔 Docker Compose 可以直接使用仓库里的 [docker-compose.ghcr.yml](docker-compose.ghcr.yml)。这个文件默认拉取 `ghcr.io/dayou0168/telegram-ledger-bot-go:2.4.14` 和 `ghcr.io/dayou0168/telegram-ledger-chain-watcher:2.4.14`，在同一个 Compose 项目里用独立 PostgreSQL 容器和独立 watcher 容器组成全家桶。在 Release 可用前不要在生产执行该模板。
...
如果希望 `ledger-chain-watcher` 直接跑在宿主机 systemd 里，v2.4.14 Release 仍会提供配套宿主机包；但本次功能不改变 watcher 协议，现有 v2.4.13 watcher 可以保持运行，只升级机器人镜像即可。
...
Go v2.4.14 通过共享 `ledger-chain-watcher` 获取链上数据。多个机器人实例只配置内网 URL 和内部密钥，不再各自配置官网 API Key：
...
Windows 发布线程使用 [scripts/publish-release.ps1](scripts/publish-release.ps1)。脚本要求 clean 的 `codex/` 集成分支、HTTPS origin、仓库局部 OpenSSL、Git Credential Manager 和有效的 `gh` OAuth；它只将候选安全快进到远端 `master`，不会创建远端候选分支，并在结束时验证远端 heads 仍精确只有 `master`。脚本不会读取或输出凭据，也不会连接生产服务器。
...
普通 push 只运行 CI。脚本只有在 PostgreSQL CI、race 和静态构建全部成功后才调用 `workflow_dispatch(version=2.4.14)`；任一步失败都会停止，不会自动部署。

服务器部署见 [docs/deployment.md](docs/deployment.md)。v2.4.14 必须在显式发布 workflow 成功且机器人镜像 digest 可用后，再按 [生产升级、回滚与验收](docs/production-rollout-v2.4.14.md) 逐个升级机器人；本轮不要重启 watcher。外层旧工作区部署文件不得回灌，唯一候选基线是本集成仓库的已确认发布提交。
