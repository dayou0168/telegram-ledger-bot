...
# v2.4.4 production rollout helper. The default action is read-only preflight.
...
backup_all() {
  ...
  discover_pg_tools
  umask 077
  local dir="$BACKUP_ROOT/v2.4.4-$(date +%Y%m%d-%H%M%S)"
...
acceptance() {
  ...
  log "checking v2.4.4 schema"
  local bot_schema watcher_schema image_id repo_digests actual_watcher_sha unauth_code
  bot_schema="$(PGDATABASE="$BOT_DATABASE_URL" "$PSQL" -Atqc "select count(*) from schema_migrations where version='2.4.4-broadcast-group-ownership'; select count(*) from information_schema.columns where table_schema='public' and table_name='broadcast_groups' and column_name='owner_user_id'; select count(*) from information_schema.tables where table_schema='public' and table_name in ('broadcast_group_owner_repair_candidates','broadcast_group_audit_events'); select count(*) from pg_indexes where schemaname='public' and indexname='idx_broadcast_groups_owner';")"
  [[ "$bot_schema" == $'1\n1\n2\n1' ]] || die "bot v2.4.4 broadcast-group ownership migration objects missing"
