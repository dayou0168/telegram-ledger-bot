...
本文只描述尚未发布的 v2.4.3 候选版本。GitHub 上一个确认发布版本是 v2.4.2；取得正式 Release、镜像 digest、宿主机二进制 SHA256 和部署授权前，不执行升级。发布内容摘要见 [releases/v2.4.3.md](releases/v2.4.3.md)；普通 push/PR 只跑 CI，不会产生 tag、Release 或镜像。

唯一代码基线是 `codex/v2.4.3-integration` 的已确认提交。外层旧工作区中的部署文件、旧权限文案和未跟踪 TRON 安装脚本不得直接合并或投产。
...
## v2.4.3 migration and recovery gates

- Back up every bot database and the watcher database before replacing either runtime.
- Deploy the watcher first, wait for `/healthz`, `/readyz`, and authenticated `/status`, then deploy every bot.
- The watcher database must record migration `2.4.3`, expose `chain_watcher_gap_tasks.head_event_id`, and contain the gap claim and overlap indexes.
- Each bot database must record `2.4.3-broadcast-permission-restore` and contain `broadcast_operator_permission_snapshots`.
- Permission hierarchy normalization is forward-only. Disabled identities retain snapshots until an explicitly authorized re-enable restores valid scopes.
- Any production-specific permission recovery SQL is an out-of-repository handoff artifact. Verify its separately supplied SHA256, run it with `psql -v ON_ERROR_STOP=1`, and never substitute IDs or apply it to another database.
...
```bash
export ROLLBACK_DIR=/root/ledger-upgrade-backups/v2.4.3-YYYYmmdd-HHMMSS
...
- bot DB 出现 `2.4.3-broadcast-permission-restore`、权限快照表和既有账期字段；watcher DB 出现 migration `2.4.3`、`chain_watcher_gap_tasks.head_event_id`、gap claim/overlap indexes 与 `chain_watcher_metric_minutes`。
