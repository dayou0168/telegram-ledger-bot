...
- Current source release candidate: `v2.4.8`
- Last confirmed published release before this candidate: `v2.4.7`
- The v2.4.8 release commit and URL do not exist until the explicit release workflow succeeds. Do not invent them from a local commit.

Intended v2.4.8 images after the explicit release workflow succeeds:

```text
ghcr.io/dayou0168/telegram-ledger-bot-go:2.4.8
ghcr.io/dayou0168/telegram-ledger-chain-watcher:2.4.8
```
...
- v2.4.7 is the last confirmed GitHub release. The unpublished v2.4.8 candidate fixes watcher parent-round cancellation being misclassified as a Key transport failure. It adds no migration and does not change bot, readiness, or fallback contracts; use `docs/production-rollout-v2.4.8.md` only after the real Release exists.
...
## v2.4.8 Candidate Runtime Configuration
...
The v2.4.8 workflow publishes the required watcher update and a matching bot image for reproducibility. Verify the watcher package checksum and `image-digests.txt`, preserve the current watcher binary and configuration, upgrade only the watcher, and require `source_ready=true`. The production bot may remain on v2.4.7; historical gaps may keep `continuity_ready=false` while they converge.
