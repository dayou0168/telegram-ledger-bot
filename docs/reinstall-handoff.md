...
- Current source and release version: `v2.4.14`
- Release URL: `https://github.com/dayou0168/telegram-ledger-bot/releases/tag/v2.4.14`
- Resolve the immutable release commit and image digests from the tag, Release assets, and `image-digests.txt`; do not invent them from an unpushed local commit.

v2.4.14 images produced by the explicit release workflow:

```text
ghcr.io/dayou0168/telegram-ledger-bot-go:2.4.14
ghcr.io/dayou0168/telegram-ledger-chain-watcher:2.4.14
```
...
- v2.4.14 completes broadcast upstream context, real-media reply flow, replacement combinations, final-only receipts, per-user send/reply preferences, and fresh quick-reply authorization through migration `2.4.20-broadcast-message-preferences`; it does not change the watcher protocol.
- Ignore deployment files from the outer legacy worktree. The only release baseline is the confirmed integration-repository commit and its matching GitHub tag.

## v2.4.14 Runtime Configuration
...
The v2.4.14 workflow publishes matching bot and watcher artifacts. For an existing v2.4.13 production installation, back up each bot database, verify the bot digest, and recreate each bot on v2.4.14; do not restart the watcher for this bot-only release.
