...
- Current source release candidate: `v2.4.9`
- Last confirmed published release before this candidate: `v2.4.8`
- The v2.4.9 release commit and URL do not exist until the explicit release workflow succeeds. Do not invent them from a local commit.

Intended v2.4.9 images after the explicit release workflow succeeds:

```text
ghcr.io/dayou0168/telegram-ledger-bot-go:2.4.9
ghcr.io/dayou0168/telegram-ledger-chain-watcher:2.4.9
```
...
- v2.4.8 is the last confirmed GitHub release. The unpublished v2.4.9 candidate adds explicit timeout ownership, fallback window slicing, and monotonic ordinary gap cursors. It adds no migration or configuration, but both watcher and bot must be upgraded; use `docs/production-rollout-v2.4.9.md` only after the real Release exists.
...
## v2.4.9 Candidate Runtime Configuration
...
The v2.4.9 workflow publishes required watcher and bot updates. Verify both immutable digests plus the watcher package checksum, preserve the current runtime files, upgrade the watcher first, require `source_ready=true`, and then recreate every bot on v2.4.9. Historical gaps may keep `continuity_ready=false` while they converge.
