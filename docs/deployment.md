...
关闭自动日切必须保存明确值 `cutoff_hour=-1`，群内 `设置日切-1` 与 `关闭日切` 完全等价。群仍在记账时关闭日切会延续当前账期并跨北京时间午夜保持 started；已经停止或账期已经失效的群不会因此自动开始。

运维禁止把历史 `cutoff_hour=0` 批量迁移为 `-1`：旧版“关闭日切”数据与用户明确设置的 00:00 日切使用了相同值，数据库无法可靠区分。恢复旧群时必须先逐群确认，只允许让确认过的群重新发送 `设置日切-1` / `关闭日切`，或按确认白名单执行带 `chat_id` 条件的定向修正；不得使用无白名单的全表更新。
