# Telegram 记账机器人 Go v2.3.5 部署运维

当前发布目标是 Go v2.3.5。生产部署使用 GHCR 预构建镜像、PostgreSQL 和共享链上监听服务 `ledger-chain-watcher`。服务器上不需要源码构建作为默认路径。

## 部署基线

- 机器人镜像：`ghcr.io/dayou0168/telegram-ledger-bot-go:2.3.5`
- watcher 镜像：`ghcr.io/dayou0168/telegram-ledger-chain-watcher:2.3.5`
...
下载 v2.3.5 发布包并安装二进制到固定路径：

```bash
cd /tmp
wget -O ledger-chain-watcher-v2.3.5-linux-amd64.tar.gz \
  https://github.com/dayou0168/telegram-ledger-bot/releases/download/v2.3.5/ledger-chain-watcher-v2.3.5-linux-amd64.tar.gz
tar -xzf ledger-chain-watcher-v2.3.5-linux-amd64.tar.gz
install -m 0755 ledger-chain-watcher-v2.3.5-linux-amd64/ledger-chain-watcher /usr/local/bin/ledger-chain-watcher
...
ledger-ops                  第二个独立 Go v2.3.5 机器人实例
```
...
v2.3.5 第一版 watcher 统一请求 Tronscan/TronGrid。未来切换自建节点时，机器人配置不变，只调整 watcher：
