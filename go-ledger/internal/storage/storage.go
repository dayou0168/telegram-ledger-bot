...
func (s *Store) migrate(ctx context.Context) error {
	...
	statements := []string{
		...
		`ALTER TABLE broadcast_groups ADD COLUMN IF NOT EXISTS id BIGINT GENERATED BY DEFAULT AS IDENTITY`,
		`CREATE UNIQUE INDEX IF NOT EXISTS idx_broadcast_groups_id ON broadcast_groups(id)`,
		...
		`ALTER TABLE notification_outbox ADD COLUMN IF NOT EXISTS payload_type TEXT NOT NULL DEFAULT 'text'`,
		`ALTER TABLE notification_outbox ADD COLUMN IF NOT EXISTS file_id TEXT NOT NULL DEFAULT ''`,
		`ALTER TABLE notification_outbox ADD COLUMN IF NOT EXISTS caption TEXT NOT NULL DEFAULT ''`,
		`ALTER TABLE notification_outbox ADD COLUMN IF NOT EXISTS reply_to_upstream_id BIGINT NOT NULL DEFAULT 0`,
		`ALTER TABLE notification_outbox ADD COLUMN IF NOT EXISTS sent_message_id BIGINT NOT NULL DEFAULT 0`,
		`CREATE TABLE IF NOT EXISTS telegram_broadcast_targets (
			...
			CHECK(mode IN ('all', 'group', 'chat')),
			CHECK((mode='all' AND chat_id=0 AND group_id IS NULL) OR
			      (mode='group' AND chat_id=0 AND group_id IS NOT NULL) OR
			      (mode='chat' AND chat_id<>0 AND group_id IS NULL))
		)`,
		`CREATE INDEX IF NOT EXISTS idx_telegram_broadcast_targets_group
			ON telegram_broadcast_targets(group_id, stream_key, user_id)
			WHERE group_id IS NOT NULL`,
		`CREATE INDEX IF NOT EXISTS idx_telegram_broadcast_targets_chat
			ON telegram_broadcast_targets(chat_id, stream_key, user_id)
			WHERE chat_id<>0`,
		`CREATE TABLE IF NOT EXISTS broadcast_upstream_messages (
			id BIGSERIAL PRIMARY KEY,
			source_operator_user_id BIGINT NOT NULL,
			source_chat_id BIGINT NOT NULL,
			source_message_id BIGINT NOT NULL,
			...
			created_at TIMESTAMPTZ NOT NULL,
			sent_at TIMESTAMPTZ,
			UNIQUE(source_chat_id, source_message_id, recipient_user_id)
		)`,
		`CREATE INDEX IF NOT EXISTS idx_broadcast_upstream_source
			ON broadcast_upstream_messages(source_chat_id, source_message_id, recipient_user_id)`,
		`CREATE INDEX IF NOT EXISTS idx_broadcast_upstream_cleanup
			ON broadcast_upstream_messages(sent_at, id) WHERE sent_at IS NOT NULL`,
	...
	if _, err := tx.Exec(ctx, `INSERT INTO schema_migrations(version, applied_at)
		VALUES($1, NOW())
		ON CONFLICT(version) DO NOTHING`, operatorMessageObserversMigrationVersion); err != nil {
		return fmt.Errorf("record operator message observers migration: %w", err)
	}
	if _, err := tx.Exec(ctx, `INSERT INTO schema_migrations(version, applied_at)
		VALUES($1, NOW())
...
func PrivateCleanupScopes(scope string) []string {
	allowed := map[string]bool{"broadcast": true, "quick_reply": true, "menu": true, "broadcast_copy": true, "broadcast_reply": true}
	...
	for _, item := range []string{"broadcast", "quick_reply", "menu", "broadcast_copy", "broadcast_reply"} {
		if seen[item] {
...
func (s *Store) RenameBroadcastGroup(ctx context.Context, oldName, newName string, actorUserID int64, manageAll bool, now time.Time) (bool, []int64, error) {
	...
	if _, err := tx.Exec(ctx, `UPDATE telegram_broadcast_targets target
		SET group_id=replacement.id,target_name=$2,updated_at=$3
		FROM broadcast_groups original, broadcast_groups replacement
		WHERE original.name=$1 AND replacement.name=$2 AND target.group_id=original.id`, oldName, newName, now); err != nil {
		return false, nil, err
	}
	rows, err := tx.Query(ctx, `SELECT user_id FROM broadcast_operator_permissions
...
func (s *Store) listBroadcastGroups(ctx context.Context, userID int64, all bool) ([]BroadcastGroup, error) {
	rows, err := s.pool.Query(ctx, `SELECT bg.id, bg.name, bg.created_by, COALESCE(bg.owner_user_id, 0),
	...
	for rows.Next() {
		...
		var chatID int64
		var title string
		if err := rows.Scan(&groupID, &name, &createdBy, &ownerUserID, &ownerUsername, &ownerDisplayName,
		...
		if group == nil {
			group = &BroadcastGroup{
				ID: groupID, Name: name, CreatedBy: createdBy, OwnerUserID: ownerUserID,
...
func (s *Store) GetBroadcastGroup(ctx context.Context, name string) (BroadcastGroup, bool, error) {
	...
	err := s.pool.QueryRow(ctx, `SELECT bg.id, bg.name, bg.created_by, COALESCE(bg.owner_user_id, 0),
		COALESCE(owner_identity.username, ''), COALESCE(owner_identity.display_name, ''),
		COALESCE(owner.remark, ''), COALESCE(owner.status, ''), bg.created_at, bg.updated_at
		FROM broadcast_groups bg
		LEFT JOIN global_operators owner ON owner.user_id=bg.owner_user_id
		...
		&group.ID, &group.Name, &group.CreatedBy, &group.OwnerUserID, &group.OwnerUsername,
...
func (s *Store) GetBroadcastGroupByID(ctx context.Context, id int64) (BroadcastGroup, bool, error) {
	if id <= 0 {
		return BroadcastGroup{}, false, nil
	}
	var name string
	err := s.pool.QueryRow(ctx, `SELECT name FROM broadcast_groups WHERE id=$1`, id).Scan(&name)
	if errors.Is(err, pgx.ErrNoRows) {
		return BroadcastGroup{}, false, nil
	}
	if err != nil {
		return BroadcastGroup{}, false, err
	}
	return s.GetBroadcastGroup(ctx, name)
}
...
func (s *Store) EnqueueNotification(ctx context.Context, item NotificationOutbox, now time.Time) (bool, error) {
	...
	tag, err := s.pool.Exec(ctx, `INSERT INTO notification_outbox(
			...
			reply_to_message_id, reply_to_upstream_id, reply_markup_json, reference_kind, reference_id, priority,
...
func (s *Store) ClaimDueNotifications(ctx context.Context, limit int, maxAttempts int, now time.Time) ([]NotificationOutbox, error) {
	...
	rows, err := s.pool.Query(ctx, `WITH next AS (
			...
			WHERE ((n.status IN ('pending', 'failed') AND n.next_attempt_at <= $1)
				...
				AND n.attempts < $4
				AND (n.reply_to_upstream_id=0 OR NOT EXISTS (
					SELECT 1 FROM broadcast_upstream_messages upstream
					JOIN notification_outbox dependency ON dependency.id=upstream.outbox_id
					WHERE upstream.id=n.reply_to_upstream_id
					  AND upstream.telegram_message_id=0
					  AND dependency.status IN ('pending','sending','failed')
					  AND dependency.attempts<$4
				))
		...
		FROM next
		WHERE n.id = next.id
		RETURNING n.id, n.kind, n.dedupe_key, n.chat_id, n.text, n.payload_type, n.file_id, n.caption, n.parse_mode,
			n.disable_preview, n.reply_to_message_id, n.reply_to_upstream_id, n.reply_markup_json,
...
func (s *Store) MarkNotificationSent(ctx context.Context, id int64, messageID int64, now time.Time) error {
	_, err := s.pool.Exec(ctx, `WITH marked AS (
			...
			SET status='sent', sent_at=$3, sent_message_id=$2, updated_at=$3, last_error=''
			WHERE id=$1
			...
			WHERE marked.reference_kind='ledger_record'
				...
				AND $2::BIGINT > 0::BIGINT
			RETURNING r.id
		)
		UPDATE broadcast_upstream_messages upstream
		SET telegram_message_id=$2,sent_at=$3
		FROM marked
		WHERE marked.reference_kind='broadcast_upstream'
		  AND marked.reference_id=upstream.id
		  AND $2::BIGINT>0`, id, messageID, now)
	return err
}
...
func (s *Store) MarkNotificationTerminalFailed(ctx context.Context, id int64, maxAttempts int, message string, now time.Time) error {
	if maxAttempts < 1 {
		maxAttempts = 1
	}
	message = trimError(message, 1000)
	_, err := s.pool.Exec(ctx, `UPDATE notification_outbox
		SET status='failed',attempts=GREATEST(attempts,$2),last_error=$3,next_attempt_at=$4,updated_at=$4
		WHERE id=$1`, id, maxAttempts, message, now)
	return err
}
