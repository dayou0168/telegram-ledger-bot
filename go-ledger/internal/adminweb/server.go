...
type pageData struct {
	...
	CanManageMessageObservers      bool
...
func (s *Server) saveMessageObserver(w http.ResponseWriter, r *http.Request) {
	...
	session := adminSessionFromContext(r.Context())
	if !s.adminCanManageGlobal(session) || !s.perms.CanManageMessageObservers(session.UserID) {
...
func (s *Server) saveReplyNotificationPreference(w http.ResponseWriter, r *http.Request) {
	...
	var selected replyNotificationTarget
	found := false
	...
	if !found {
		http.Error(w, "不能设置该操作人的发送或回复通知", http.StatusForbidden)
		return
	}
	receiveBroadcast := r.FormValue("receive_broadcast") == "1"
	receiveReply := r.FormValue("receive_reply") == "1"
	if (receiveBroadcast && !selected.AllowBroadcast) || (receiveReply && !selected.AllowReply) {
		http.Error(w, "所选通知超出当前消息观察授权范围", http.StatusForbidden)
		return
	}
	if err := s.store.SetBroadcastMessagePreference(r.Context(), session.UserID, sourceUserID, receiveBroadcast, receiveReply, time.Now()); err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
	redirectAdminTab(w, r, "replies", "发送/回复通知设置已保存")
}
...
func (s *Server) loadPageData(ctx context.Context, message string, session adminauth.Session, filterArgs ...adminListFilters) (pageData, error) {
	...
	canManageMessageObservers := canManageGlobal && s.perms.CanManageMessageObservers(session.UserID)
	...
	var (
		...
		messageSources      []storage.GlobalOperator
		messagePrimaries    []storage.GlobalOperator
	)
	...
	if canManageMessageObservers {
		...
		for _, op := range allOperators {
			...
			switch op.Level {
			case "secondary":
				messageSources = append(messageSources, op)
			case "primary":
				messagePrimaries = append(messagePrimaries, op)
			}
		}
	}
	if canManageGlobal {
	...
	return pageData{
		...
		MessageObserverSources:         messageSources,
		MessageObserverPrimaries:       messagePrimaries,
		...
		CanManageMessageObservers:      canManageMessageObservers,
		ChatNames:                      chatNames,
		OpLabels:                       opLabels,
...
func (s *Server) replyNotificationTargets(ctx context.Context, session adminauth.Session, caps permissions.UserCapabilities) ([]replyNotificationTarget, string, bool, error) {
	isHost := s.perms.IsHost(session.UserID)
	if !isHost && caps.GlobalOperatorLevel != "primary" {
	...
	operatorByID := make(map[int64]storage.GlobalOperator, len(operators))
	for _, op := range operators {
		operatorByID[op.UserID] = op
	}
	scopes, err := s.store.ListOperatorMessageSourcesForObserver(ctx, session.UserID, s.perms.HostUserID())
	if err != nil {
		return nil, "", false, err
	}
	overrides, err := s.store.BroadcastMessagePreferenceOverrides(ctx, session.UserID)
	if err != nil {
		return nil, "", false, err
	}
	targets := make([]replyNotificationTarget, 0, len(scopes))
	for _, sourceScope := range scopes {
		op, exists := operatorByID[sourceScope.SourceUserID]
		if !exists || op.Status != "active" {
			continue
		}
		relation := operatorLevelLabel(op.Level)
		if !isHost {
			relation = "消息观察授权"
			if op.ParentUserID == session.UserID {
				relation = "直属下级操作人"
			}
		}
		target := replyNotificationTarget{
			UserID: op.UserID, Label: operatorLabel(op), Relation: relation,
			AllowBroadcast: sourceScope.AllowBroadcast, AllowReply: sourceScope.AllowReply,
			ReceiveBroadcast: sourceScope.AllowBroadcast, ReceiveReply: sourceScope.AllowReply,
		}
		if preference, exists := overrides[op.UserID]; exists {
			target.ReceiveBroadcast = sourceScope.AllowBroadcast && preference.ReceiveBroadcast
			target.ReceiveReply = sourceScope.AllowReply && preference.ReceiveReply
		}
	...
	scope := "可分别设置是否接收每位操作人的广播内容和群内回复。"
	if !isHost {
		scope = "只能设置直属下级及宿主授权观察的下级；开关不会扩大消息观察范围。"
	}
...
func messageObserverLabel(userID int64, labels map[int64]string) string {
...
var adminTemplate = template.Must(template.New("admin").Funcs(template.FuncMap{
	...
	"messageObserverLabel":        messageObserverLabel,
}).Parse(adminHTML))
...
.reply-settings{max-height:min(620px,calc(100vh - 260px));overflow-y:auto;overscroll-behavior:contain;border:1px solid #dce5ef;border-radius:8px;background:#fff}.reply-row{display:grid;grid-template-columns:minmax(180px,1fr) minmax(150px,.65fr) minmax(300px,1.2fr) auto;gap:12px;align-items:center;min-width:0;padding:12px 14px;border-bottom:1px solid #e6edf5}.reply-row:last-child{border-bottom:0}.reply-name{min-width:0;font-weight:800;overflow-wrap:anywhere}.reply-relation{min-width:0;color:var(--muted);overflow-wrap:anywhere}.reply-controls{display:grid;grid-template-columns:repeat(2,minmax(130px,1fr));gap:8px;min-width:0}.reply-channel{min-width:0;padding:7px 9px;border:1px solid #dce5ef;border-radius:6px;background:#f8fafc}.reply-channel-title{display:block;margin-bottom:5px;color:#40566f;font-size:12px;font-weight:800}.reply-toggle{position:relative;display:flex;align-items:center;gap:7px;min-width:0;white-space:nowrap;cursor:pointer}.reply-toggle input{position:absolute;opacity:0;pointer-events:none}.reply-toggle-track{position:relative;flex:0 0 40px;width:40px;height:22px;border-radius:999px;background:#aab7c6;transition:.18s}.reply-toggle-track::after{content:"";position:absolute;top:3px;left:3px;width:16px;height:16px;border-radius:50%;background:#fff;box-shadow:0 1px 3px rgba(0,0,0,.25);transition:.18s}.reply-toggle input:checked+.reply-toggle-track{background:var(--blue)}.reply-toggle input:checked+.reply-toggle-track::after{transform:translateX(18px)}.reply-toggle input:focus-visible+.reply-toggle-track{outline:3px solid rgba(45,108,223,.25);outline-offset:2px}.reply-status{min-width:42px;color:var(--muted);font-size:12px;font-weight:700}.reply-unavailable{color:var(--muted);font-size:12px}.reply-empty{padding:24px;text-align:center;color:var(--muted)}
.observer-editor{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr) minmax(220px,.8fr) auto;gap:10px;align-items:end;margin-bottom:14px}.observer-editor select{width:100%}.observer-channels{min-width:0;display:flex;gap:14px;align-items:center;min-height:38px;margin:0;padding:7px 10px;border:1px solid #dce5ef;border-radius:6px}.observer-channels label{display:flex;gap:6px;align-items:center;white-space:nowrap}.observer-channels input{width:auto;min-height:auto}.observer-grants{max-height:min(520px,calc(100vh - 310px));overflow:auto;overscroll-behavior:contain;border:1px solid #dce5ef;border-radius:6px}.observer-grants table{margin:0}.observer-channel-list{display:flex;gap:6px;flex-wrap:wrap;justify-content:center}.observer-empty{padding:18px;color:var(--muted);text-align:center;border:1px dashed #cbd8e8;border-radius:6px;background:var(--soft)}
@media(max-width:900px){.top{align-items:flex-start;flex-direction:column}.row,.row.two,.group-operations,.member-grid,.permission-panels,.permission-panel form,.observer-editor{grid-template-columns:1fr}.group-operations{grid-template-areas:"create" "edit" "danger"}.group-operation-edit{min-height:0;border-left:0;border-top:1px solid #d7e1ec}.group-operation-desc{min-height:0}.watch-head{display:none}.watch-row{grid-template-columns:1fr}.reply-settings{max-height:calc(100vh - 230px)}.reply-row{grid-template-columns:minmax(0,1fr)}.reply-relation,.reply-controls,.reply-row .btn{grid-column:1}.reply-row .btn{width:100%}.observer-editor .btn{width:100%}.observer-grants{max-height:calc(100vh - 270px)}.btn{width:100%}.table-tools{flex-wrap:nowrap}.table-tools input[type=search]{flex:1 1 0;width:0;min-width:0}.table-tools .btn{flex:0 0 auto;width:auto;min-width:68px}.cleanup-dialog-body{grid-template-columns:1fr}.cleanup-dialog-body .wide{grid-column:auto}.cleanup-dialog-actions .btn,.cleanup-time .btn{width:auto}.cleanup-cell{min-width:140px;max-width:190px}.cleanup-summary{max-width:125px}}
...
<button class="tab-btn" type="button" data-admin-tab-target="replies">发送/回复通知</button>
{{end}}
{{if .CanManageMessageObservers}}
<button class="tab-btn" type="button" data-admin-tab-target="observers">消息观察授权</button>
{{end}}
</nav>
...
<h2>广播发送/回复通知</h2>
<p class="hint">{{.ReplyNotificationScope}}关闭只影响当前登录人的私聊收件，不撤销宿主设置的观察授权，也不影响广播实际投递；广播发送人始终收到自己广播的群内回复。</p>
...
<div class="reply-channel"><span class="reply-channel-title">接收发送</span>{{if .AllowBroadcast}}<label class="reply-toggle"><input type="checkbox" name="receive_broadcast" value="1" {{if .ReceiveBroadcast}}checked{{end}}><span class="reply-toggle-track" aria-hidden="true"></span><span class="reply-status">{{if .ReceiveBroadcast}}接收{{else}}关闭{{end}}</span></label>{{else}}<span class="reply-unavailable">观察授权未开放</span>{{end}}</div>
<div class="reply-channel"><span class="reply-channel-title">接收回复</span>{{if .AllowReply}}<label class="reply-toggle"><input type="checkbox" name="receive_reply" value="1" {{if .ReceiveReply}}checked{{end}}><span class="reply-toggle-track" aria-hidden="true"></span><span class="reply-status">{{if .ReceiveReply}}接收{{else}}关闭{{end}}</span></label>{{else}}<span class="reply-unavailable">观察授权未开放</span>{{end}}</div>
</div>
...
{{if .CanManageMessageObservers}}
<div class="card wide tab-card" data-admin-tab="observers">
<h2>消息观察授权</h2>
<p class="hint">仅控制跨一级查看下级操作人的广播内容与群内回复，不授予广播目标、分组管理、下级管理或其他后台数据权限。宿主和直属一级仍按默认关系接收。</p>
{{if and .MessageObserverSources .MessageObserverPrimaries}}
<form method="post" action="/admin/message-observer/save" class="observer-editor">
<label class="field-stack"><span class="field-label">来源下级</span><select class="observer-source" name="source_secondary_user_id">{{range .MessageObserverSources}}<option value="{{.UserID}}" data-parent-user-id="{{.ParentUserID}}">{{operatorLabel .}}</option>{{end}}</select></label>
<label class="field-stack"><span class="field-label">观察一级</span><select class="observer-primary" name="observer_primary_user_id">{{range .MessageObserverPrimaries}}<option value="{{.UserID}}">{{operatorLabel .}}</option>{{end}}</select></label>
<fieldset class="observer-channels"><legend class="field-label">接收内容</legend><label><input type="checkbox" name="receive_broadcast" value="1" checked>广播内容</label><label><input type="checkbox" name="receive_reply" value="1" checked>群内回复</label></fieldset>
<button class="btn" type="submit">启用 / 更新授权</button>
</form>
{{else}}<div class="observer-empty">需要至少一个有效下级和一个非直属的有效一级操作人。</div>{{end}}
<div class="observer-grants"><table><thead><tr><th>来源下级</th><th>观察一级</th><th>接收内容</th><th>状态</th><th>操作</th></tr></thead><tbody>
{{range .MessageObserverGrants}}<tr><td>{{messageObserverLabel .SourceSecondaryUserID $.OpLabels}}</td><td>{{messageObserverLabel .ObserverPrimaryUserID $.OpLabels}}</td><td><span class="observer-channel-list">{{if .ReceiveBroadcast}}<span class="pill">广播</span>{{end}}{{if .ReceiveReply}}<span class="pill">回复</span>{{end}}</span></td><td>{{if .Active}}<span class="pill">启用</span>{{else}}已撤销{{end}}</td><td>{{if .Active}}<form method="post" action="/admin/message-observer/revoke"><input type="hidden" name="source_secondary_user_id" value="{{.SourceSecondaryUserID}}"><input type="hidden" name="observer_primary_user_id" value="{{.ObserverPrimaryUserID}}"><button class="btn mini secondary" type="submit">撤销</button></form>{{else}}通过上方重新启用{{end}}</td></tr>{{else}}<tr><td colspan="5">暂无跨一级消息观察授权</td></tr>{{end}}
</tbody></table></div>
</div>
{{end}}
...
document.querySelectorAll('.observer-editor').forEach(function(form){
  const source=form.querySelector('.observer-source');
  const observer=form.querySelector('.observer-primary');
  function syncObserverOptions(){
    if(!source || !observer){return;}
    const selected=source.options[source.selectedIndex];
    const parentID=selected ? selected.dataset.parentUserId : '';
    Array.from(observer.options).forEach(function(option){option.disabled=option.value===parentID;});
    if(observer.selectedOptions.length && observer.selectedOptions[0].disabled){
      const next=Array.from(observer.options).find(function(option){return !option.disabled;});
      observer.value=next ? next.value : '';
    }
  }
  if(source){source.addEventListener('change',syncObserverOptions);}
  syncObserverOptions();
});
