# Telegram Ledger Bot Go Runtime v2.4.14

这是机器人 Go 版 v2.4.14 发布主线，目标是把同步、异步、队列、并发、缓存、数据库和共享链上监听从架构层面重新设计。当前已具备：

发布说明见 [../docs/releases/v2.4.14.md](../docs/releases/v2.4.14.md)。
...
- 广播发送与回复通知分别保存个人偏好，并受 observer ceiling 约束；上级抄送包含发送人和真实目标，群成员回复回流保留原上下文与真实媒体。
- 广播替换：后台可开关，仅对单群发送生效；纯文字、图片和 caption 原消息都可按固定图片/文字矩阵替换。
...
- v2.4.14 链上监听继续通过共享 `ledger-chain-watcher` 获取链上数据；现有 v2.4.13 watcher 与本次机器人功能兼容，无需重启。机器人侧继续保存监听地址、event identity 去重和 Telegram outbox。
...
正式发布后，推荐直接用仓库根目录的 `docker-compose.yml` 或 `docker-compose.ghcr.yml` 启动，同一个 Compose 项目里包含 PostgreSQL 独立容器和 `ledger-chain-watcher` 独立容器，默认拉取 `ghcr.io/dayou0168/telegram-ledger-bot-go:2.4.14` 与 `ghcr.io/dayou0168/telegram-ledger-chain-watcher:2.4.14`：
...
GitHub Release `v2.4.14` 同时发布配套 watcher 宿主机包，但本次修复只需要把机器人升级到 v2.4.14，生产 watcher 保持当前版本和进程不动。
...
- PostgreSQL 是 v2.4.14 的唯一主库目标，避免后续再次迁移。

## 启动原则

Go v2.4.14 直接按 PostgreSQL 空库启动。等账单、广播、监听三块经过真实群测试后，再切换生产 Bot Token。多机器人部署时，每个机器人实例独立 PostgreSQL；只有 `ledger-chain-watcher` 共享。
