# Telegram Ledger Bot Go Runtime v2.4.9

这是机器人 Go 版 v2.4.9 发布候选主线，目标是把同步、异步、队列、并发、缓存、数据库和共享链上监听从架构层面重新设计。当前已具备：

候选版发布说明见 [../docs/releases/v2.4.9.md](../docs/releases/v2.4.9.md)。

v2.4.9 用显式 timeout cause 区分父轮次取消与独立网络超时，修复 fallback window 切片边界，并为普通 gap cursor 增加单调写入防线。真实请求超时仍冷却 Key；只有专用 Split 事务可重置子任务 cursor。本补丁无迁移、无配置变化。
...
- v2.4.9 链上监听继续通过共享 `ledger-chain-watcher` 获取链上数据；watcher 与 bot 均有运行时改动，必须同步升级。机器人侧继续保存监听地址、event identity 去重和 Telegram outbox。
...
正式发布后，推荐直接用仓库根目录的 `docker-compose.yml` 或 `docker-compose.ghcr.yml` 启动，同一个 Compose 项目里包含 PostgreSQL 独立容器和 `ledger-chain-watcher` 独立容器，默认拉取 `ghcr.io/dayou0168/telegram-ledger-bot-go:2.4.9` 与 `ghcr.io/dayou0168/telegram-ledger-chain-watcher:2.4.9`：
...
GitHub Release `v2.4.9` 同时发布 `ledger-chain-watcher-v2.4.9-linux-amd64.tar.gz` 宿主机包，里面包含 Linux amd64 二进制和上述两个模板。本轮先更新 watcher 并确认 `source_ready=true`，再把所有 bot 同步升级到 v2.4.9；历史 gap 可继续后台收敛。
...
- PostgreSQL 是 v2.4.9 的唯一主库目标，避免后续再次迁移。

## 启动原则

Go v2.4.9 直接按 PostgreSQL 空库启动。等账单、广播、监听三块经过真实群测试后，再切换生产 Bot Token。多机器人部署时，每个机器人实例独立 PostgreSQL；只有 `ledger-chain-watcher` 共享。
